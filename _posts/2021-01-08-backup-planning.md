...
*NOTE* - Due to the nature of this blog, the example applications I provide are FOSS and Linux-compatible, but you can apply the same concepts to other apps and operating systems.
...
Family pictures, tax documents, home videos; things that aren't replaceable or would incur significant cost to replace, financial or otherwise. These should exist in at 3 places.

### Tier 2: Files that you *can* replace, but don't want to

Music and movies, either ripped from physical media or "obtained" from elsewhere, are good contenders for this category. You may have spent hours meticulously cataloging your media collection, but those files exist elsewhere and could be re-downloaded or ripped. These should exist in at least 2 places.

### Tier 3: Files that don't matter

Installed applications and games, operating system files, miscellaneous downloads. Files in this category can be easily replaced with a simple re-install or are publicly available on the internet. Not necessary to back up these files unless you've got extra terabytes burning a hole in your pocket.
...
This is as simple as it gets: plug in a USB hard drive or connect to a network share and copy your files to it. While it's possible to accomplish this with a drag-and-drop, there are many great software tools that can help: FreeFileSync, DirSyncPro, and rsync are all great options for the desktop. 

**Advantages**

- **SImple** - Does not require extensive knowledge to set up and understand what's happening, and the applications don't hide the files behind compression or custom folder structures. Great for people who want to visual verification of a successful backup.
...
Backup applications like Borg, Duplicati, and BackupPC provide features that make your backups more efficient and resilient: multiple version retention, encryption, de-duplication, and compression to name a few. 
...
We already established the cloud as a great option for your Tier 1 files, so what makes cloud backups different from cloud sync services? Cloud backup applications use the same methodology as the local backup applications I mentioned previously, but instead of going to a USB HDD or NAS, we're sending them to cloud storage. Encryption, compression, de-duplication and versioning are features can expect to find, and many of the apps I already mentioned are cloud-compatible. Borg, Restic, and Duplicati are all great options in this space.

**Advantages**

- **Secure** - Encryption allows you to use third-party hosting services with fewer privacy and security concerns. They can't see your files, only encrypted data.
...
- **Cheaper storage options than cloud sync** - Since device sync and web availability isn't a requirement for this type of backup, you're options are opened up to any cloud storage vendor. 
...
- **Tier 1** - When your files are too large to practically store on a sync service or for use in addition to a sync service.

### Local - USB vs NAS

If you're starting from nothing, getting a USB hard drive will likely be your first step. If you don't have one lying around already, they can be purchased cheaply and are easy to plug in and use. However, if you can afford it, the ideal solution is either a pre-built NAS device (Synology, iXSystems, or QNAP) or a DIY NAS using either a traditional x86 PC or something like a Raspberry Pi or RockPro64. 

Either way, it's important to understand the advantages and limitations of both options:

**USB Hard Drives**

**Advantages**

- **Cheap** - 4TB USB hard drives are less than $100 at most retailers.
...
- **Shareable** - Since a NAS resides on your network, you can back up multiple laptops and desktops at once without having to plug in a cable. 

- **Great for laptops** - Since you don't need to plug in a hard drive, your laptop can back itself up on a schedule automatically. 
...
- **Setup and maintenance** - Whether pre-built or custom, a NAS is a full computer running on your network that needs to be configured, updated, and maintained. To minimize this, choose a device from Synology or QNAP. 

## So what should you do?

Everybody's situation is different, but there are some general conclusions you can draw from this information:

1. Tier 1 data should be backed up both locally and with a cloud sync service.

2. Tier 2 data should be backed up locally in at least one additional location.

3. Local Backups are preferred over Local Syncs for their reliability and resiliency.

4. If you can't maintain a local backup, combine a cloud sync service with a cloud backup option.

5. The most important thing is that you do something. Your data is too valuable and your computer's hard drive is too volatile. 

## Bonus: My Setup

- Some **Tier 1** data is synced with a Nextcloud server hosted on DigitalOcean and that server is backed up to my local NAS using Borg and Borgmatic.

- All **Tier 1** data on my NAS is backed up nightly to BorgBase using Borg and Borgmatic.

- **Tier 2** data on my NAS is backed up nightly to external hard drives using ZFS and Sanoid/Syncoid.
